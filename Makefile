...
checkall:
		@test -d build || echo "Can't run tests: no build folder\n"
		@test -d build && pushd build && ./subjectivec_tests --success --durations yes --abortx 10 && popd

check:
		@test -d build || echo "Can't run tests: no build folder\n"
		@test -d build && pushd build && ./subjectivec_tests --durations yes --abortx 20 && popd
