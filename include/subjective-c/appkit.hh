...
namespace objc {
    ...
    namespace appkit {
        ...
        {
            static_assert(objc::traits::is_object<objc::ocpointer_t<OCType>>::value,
                          "objc::appkit::can_paste<OCType> requires an Objective-C object type");
            
            if (!board) { board = [NSPasteboard generalPasteboard]; }
            
            return [board canReadObjectForClasses:@[ [objc::octype_t<OCType> class] ]
                                          options:@{}];
        }
        ...
        objc::ocpointer_t<OCType> paste(NSPasteboard* board = nil) noexcept
        {
            ...
            if (!board) { board = [NSPasteboard generalPasteboard]; }
            if (!can_paste<objc::ocpointer_t<OCType>>(board)) { return nil; }
